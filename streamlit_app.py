...
async def download_files_and_run():
    ...
    try:
        ...
        with open(config_path, 'wb') as f:
            for chunk in response.iter_content(chunk_size=8192):
                f.write(chunk)
        print("Download config.json successfully")
    except Exception as e:
        print(f"Download config.json failed: {e}, using built-in config generation")
        # 原有的配置文件生成代码（作为备用）
        config = {
            ...
            "dns": {
                "servers": ["https+local://doh-4wr.pages.dev/chaxun"],
                "queryStrategy": "UseIP"
            },
            "outbounds": [
                {
                ...
                },
                {
                    ...
                    "settings": {
                        ...
                        "address": ["172.16.0.2/32", "2606:4700:110:8d67:252e:1624:cfae:59ef/128"],
                        "peers": [
                            {
            ...
            "routing": {
                ...
                "rules": [{"type": "field", "ip": ["::/0"], "outboundTag": "WARP"}]
            }
        }
    ...
    # Run sbX
    command = f"nohup {os.path.join(FILE_PATH, 'web')} -c {config_path} >/dev/null 2>&1 &"
    try:
        exec_cmd(command)
